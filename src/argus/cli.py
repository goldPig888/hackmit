...
def demo(output: Path) -> None:
    pipeline, publisher = ArgusPipeline(), HapticPublisher(output)
    print("ARGUS simulated crossing scenario (rider: 4 m/s, car approaches rear-left)")
...
def camera(source: str, output: Path) -> None:
    ...
    model, pipeline, publisher = YOLO("yolo11n.pt"), ArgusPipeline(), HapticPublisher(output)
    ...
    while True:
        ...
        cv2.imshow("ARGUS — press q to stop", result.plot())
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ARGUS safety prototype")
