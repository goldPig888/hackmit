...
class CoordinateFrame(Enum):
    ...
    ARGUS = "argus"    # ARGUS world frame (stabilized)
    WORLD = "world"    # Global world frame


@dataclass
class ObjectState:
    """Object state in stabilized ARGUS coordinate frame.
    
    ARGUS coordinate frame:
    - +Y: Forward direction
    - +X: Right direction  
    - +Z: Up direction
    """
    ...
    # Position in ARGUS frame [X, Y, Z] (meters)
    position: np.ndarray
    
    # Velocity in ARGUS frame [Vx, Vy, Vz] (m/s)
    velocity: np.ndarray
