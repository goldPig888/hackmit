...
class LiveCameraDashboard:
    ...
    def __init__(self, camera_source: int = 0, port: int = 8080,
        ...
        """Initialize live camera dashboard.
        ...
        # Initialize data streamer for dashboard
    ...
    def process_frame(self, frame: np.ndarray, timestamp: float) -> dict:
        ...
        for detection in results['detections']:
            # Simulate IMU data (in real deployment, use actual IMU)
    ...
    def _simulate_imu_data(self, timestamp: float) -> tuple:
        ...
        Simulates different motion patterns based on active test:
        - approaching: Simulate moving forward
        - turning: Simulate yaw rate changes
        - lateral: Simulate lateral motion
        
        Args:
            timestamp: Current timestamp
            
        Returns:
            Tuple of (gyro, accel, quaternion)
        """
        # Update simulated motion based on test type
        if self.motion_test_active:
            if self.motion_test_type == "approaching":
                # Simulate approaching - slight forward acceleration
                self.simulated_speed = min(3.0, self.simulated_speed + 0.01)
                self.simulated_yaw_rate = 0.0
            elif self.motion_test_type == "turning":
                # Simulate turning - oscillating yaw rate
                ...
                # Simulate lateral motion - slight lateral acceleration
                self.simulated_yaw_rate = 0.0
                self.simulated_speed = 1.5
            else:
                # Reset to default
        ...
        # Simulate gyro (angular velocity)
        gyro = np.array([0.01, 0.02, self.simulated_yaw_rate])  # rad/s
        
        # Simulate accel (linear acceleration)
        if self.motion_test_type == "approaching":
        ...
        quaternion = np.array([cy, 0.0, 0.0, sy])  # Rotation around Z axis
    ...
    def draw_overlays(self, frame: np.ndarray, results: dict) -> np.ndarray:
        ...
        # Draw system info
        self._draw_system_info(overlay_frame, results)
        
        # Draw motion test info
        if self.motion_test_active:
            self._draw_motion_test_info(overlay_frame)
        
        return overlay_frame

    def _draw_system_info(self, frame: np.ndarray, results: dict) -> None:
        ...
        if self.use_advanced and results['system_status']:
            ...
            cv2.putText(frame, f"Objects: {status['num_tracked_objects']}", (10, 90),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(frame, f"Ego Speed: {status['ego_speed']:.1f} m/s", (10, 120),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            
            if status['is_turning']:
                cv2.putText(frame, f"Turning: {status['turn_direction']}", (10, 150),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

    def _draw_motion_test_info(self, frame: np.ndarray) -> None:
        """Draw motion test information.
        ...
        cv2.putText(frame, f"Motion Test: {self.motion_test_type}", (10, frame.shape[0] - 100),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        cv2.putText(frame, f"Elapsed: {elapsed:.1f}s", (10, frame.shape[0] - 70),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        cv2.putText(frame, "Press 'q' to stop test", (10, frame.shape[0] - 40),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
    ...
    def update_dashboard(self, results: dict, overlay_frame: np.ndarray) -> None:
        ...
        Args:
            results: Processing results
            overlay_frame: Frame with overlays
        """
    ...
    def run(self) -> None:
        """Run the live camera dashboard."""
        print("=" * 60)
        print("Live Camera Dashboard")
        print("=" * 60)
        ...
        try:
            while True:
                ...
                if self.motion_test_active and results['system_status']:
                    self.motion_history.append({
                        ...
                        'objects': len(results['system_status']['num_tracked_objects'])
                    })
            ...
            print("Camera dashboard stopped")
...
def main():
    ...
    parser.add_argument("--no-overlay", action="store_true", help="Disable visual overlays")
    
    args = parser.parse_args()
    
    try:
        dashboard = LiveCameraDashboard(
