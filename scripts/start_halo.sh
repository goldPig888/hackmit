...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --iphone)
            ...
            ;;
        --help)
            echo "HALO Unified Start Script"
            echo ""
...
# Start iPhone stream pipeline if enabled
if [ "${ENABLE_IPHONE:-false}" = true ]; then
    echo "Starting iPhone stream pipeline..."
    python scripts/live_iphone_camera.py --port "$PORT" &
    IPHONE_PID=$!
    echo "iPhone pipeline started (PID: $IPHONE_PID)"
    echo "Phone page: http://localhost:$PORT/phone"
    if [ "$ENABLE_TUNNEL" = false ]; then
        echo "NOTE: iOS needs HTTPS for camera+motion. Re-run with --tunnel."
    fi
    # The iPhone script already hosts the dashboard server on PORT.
    ENABLE_DASHBOARD=false
fi
...
echo "=========================================="
echo "HALO System Started Successfully"
echo "=========================================="
